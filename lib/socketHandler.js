...
let socketIoInitialize = function(server){
    ...
    io.sockets.on('connection', function(socket){
        ...
        // 파이선에서 디스플레이 정보를 요청하면 아래 코드가 실행 됨.
        socket.on('lcdInfo', function(message){
            let waitingCount = Waiting.find({}).count();
            let waitingTime = waitingCount*15;
            let data = {
                'waitingCount' : waitingCount,
                'waitingTime' : waitingTime
            };

            io.sockets.emit('waitingCount', data);
        });
    });
};
